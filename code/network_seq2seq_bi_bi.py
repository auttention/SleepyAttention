...
logdir = "./tensorboard_logs/seq2seq_bidirect_without_fc"
writer = tf.contrib.summary.create_file_writer(logdir)
with writer.as_default():
    tf.contrib.summary.always_record_summaries()
optimizer = tf.train.AdamOptimizer(learning_rate=LEARNING_RATE)
checkpoint_directory = './training_checkpoints/seq2seq_bidirectional'
...
class Encoder(tf.keras.Model):
    ...
    def call(self, init_hidden, encoder_input):
        ...
        return hidden, [hidden_f, hidden_b]
...
class Decoder(tf.keras.Model):
    ...
    def call(self, encoder_hidden, decoder_input):
        ...
        predictions_f, _ = self.gru_1_f(decoder_input, initial_state=encoder_hidden[0])
        predictions_b, _ = self.gru_1_b(decoder_input, initial_state=encoder_hidden[1])
        predictions = tf.concat([predictions_f, predictions_b], -1)

        predictions_f, _ = self.gru_2_f(predictions, initial_state=encoder_hidden[0])
        predictions_b, _ = self.gru_2_b(predictions, initial_state=encoder_hidden[1])
        predictions = tf.concat([predictions_f, predictions_b], -1)
...
def train_step_network(input, encoder_hidden):
    with tf.GradientTape() as tape:
        _, decoder_hidden = encoder(encoder_hidden, input)
