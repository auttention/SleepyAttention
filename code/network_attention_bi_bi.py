...
logdir = "./tensorboard_logs/attention_bi_bi"
writer = tf.contrib.summary.create_file_writer(logdir)
with writer.as_default():
    tf.contrib.summary.always_record_summaries()
optimizer = tf.train.AdamOptimizer(learning_rate=LEARNING_RATE)
checkpoint_directory = './training_checkpoints/attention_bi_bi'
...
class Decoder(tf.keras.Model):
    ...
    def call(self, x, decoder_hidden, encoder_output):
        context_vector, attention_weights = self.attention(decoder_hidden, encoder_output)

        x = tf.keras.layers.Dropout(0.2)(x)
        x = tf.concat([tf.expand_dims(context_vector, 1), x], axis=-1)

        predictions_f, _ = self.gru_1_f(x)
        predictions_b, _ = self.gru_1_b(x)
        predictions = tf.concat([predictions_f, predictions_b], -1)

        predictions_f, hidden_f = self.gru_2_f(predictions)
        predictions_b, hidden_b = self.gru_2_b(predictions)
...
def train_step_network(input, encoder_hidden):
    ...
    with tf.GradientTape() as tape:
        encoder_output, encoder_hidden = encoder(encoder_hidden, input)
        ...
        for i in range(input.shape[1]):
            ...
            projection, decoder_hidden, _ = decoder(input_slice, decoder_hidden, encoder_output)
...
def extract_reconstructed_spectrograms(dataset, spectrograms_directory, checkpoint_directory="./training_checkpoints/attention_bi_bi"):
...
def extract_features(dataset, features_path, checkpoint_directory="./training_checkpoints/attention"):
    checkpoint = tf.train.Checkpoint(optimizer=optimizer, encoder=encoder, decoder=decoder)
    checkpoint.restore(tf.train.latest_checkpoint(checkpoint_directory))
...
def extract_all_features(features_directory, features_name, checkpoint_directory="./training_checkpoints/attention"):
...
def extract_all_reconstructed_spectrograms(spectrograms_directory, checkpoint_directory="./training_checkpoints/attention"):
